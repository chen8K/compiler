...
Exp_t是exp_tree
Stack_t是栈式计算机的指令 
List_t是instruction list（即指令（Stack_t）的list） 

本程序在windows不能执行 (因为一些写法上的差异比如malloc)
*/ 

void emit (struct Stack_t *instr)//all指向一个 instruction list的第一个，这个函数创建一个 instruction并让他当这个 instruction list的头 
{
...
void compile (struct Exp_t *exp)
{
  ...
  case EXP_SUM:{
  	struct Exp_Sum *p = (struct Exp_Sum *)exp;
  	if(p->left!=NULL){
  		compile((struct Exp_t *)p->left);
	}
	if(p->right!=NULL){
		compile((struct Exp_t *)p->right);
	}
    emit(Stack_Add_new());
    break;
  }
